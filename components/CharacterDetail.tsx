...
import { api, ProfileSummary, DatasetImage, ProfileFull, UpdateProfileBody } from '../lib/api';
...
export function TrainingProgress({
  ...
  // KNOWN TRADEOFF. This used to tick every 3s, which is how the step counter
  // («⚙ kohya обучает LoRA, шаг 420/2000») advanced on screen. kohya's step
  // count is NOT a queue fact — `queue_entries` only records that a training job
  // started and that it finished — so there is no delta to push mid-run.
  //
  // Result: the phase label and counter now update when training STARTS, when it
  // FINISHES, on tab wake, and on ⟳ — not continuously. Getting the live counter
  // back needs a second push source (a kohya log tail), which is deliberately out
  // of scope. See docs/live-updates.md § «Чего сокет не знает».
  const load = useCallback(async () => {
    if (!isActive || !jobId) { setProgress(null); return; }
    try { setProgress(await api.trainingProgress(jobId)); } catch { /* ignore */ }
  }, [isActive, jobId]);

  useEffect(() => { void load(); }, [load]);

  const trainMatch = useCallback(on.all(on.job(jobId ?? null), on.types('training')), [jobId]);
  useLiveEvents(trainMatch, load, { active: isActive });
